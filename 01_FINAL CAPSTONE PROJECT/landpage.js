...
const burger = document.querySelector('.home');
...
window.onscroll = () => {
  ...
  if (this.scrollY <= 10) {
    menu.classList.remove("main--sticky");
    items.forEach(item => {
      item.classList.remove("nav__item--sticky");
    })

  }
  else {
    menu.classList.add("home--sticky");
    items.forEach(item => {
      item.classList.add("nav__item--sticky");
    })
  };
};

burger.addEventListener("click", () => {
  menu.classList.toggle("main--open");
});
